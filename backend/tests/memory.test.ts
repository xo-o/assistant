...
describe("Memory & Session Isolation Suite", () => {
  ...
  before(() => {
    repository.ensureSession(sessionA);
    repository.ensureSession(sessionB);
  });
  ...
  it("should keep concurrent sessions strictly isolated without cross-contamination", () => {
    // Record Laura's preference in Session A
    repository.saveOrUpdateLead({
      sessionId: sessionA,
      name: "Laura",
    ...
    // Verify Session A
    const leadA = repository.getLeadBySessionId(sessionA);
    const messagesA = repository.getSessionMessages(sessionA);
    assert.ok(leadA);
    assert.strictEqual(leadA.name, "Laura");
    assert.strictEqual(leadA.vehicle_type_interest, "Hatchback");
    ...
    // Verify Session B
    const leadB = repository.getLeadBySessionId(sessionB);
    const messagesB = repository.getSessionMessages(sessionB);
    assert.ok(leadB);
    assert.strictEqual(leadB.name, "Roberto");
    assert.strictEqual(leadB.vehicle_type_interest, "Pickup 4x4");
  ...
  it("should truncate message history within configured token budget using sliding window", () => {
    const budgetSession = "sess_budget_003";
    repository.ensureSession(budgetSession);
    ...
    const windowed = repository.getSlidingWindowMessages(budgetSession, 350, 10);
