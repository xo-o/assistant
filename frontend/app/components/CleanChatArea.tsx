        ...
        <div className="max-w-3xl mx-auto space-y-7">
          {messages.length === 0 ? (
            <div className="text-center py-16 px-4 max-w-xl mx-auto">
              <div className="size-12 rounded-2xl bg-card border border-border text-foreground flex items-center justify-center mx-auto mb-4 shadow-sm">
              ...
              <div className="space-y-2 text-left">
                <span className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider block px-1">
                  Preguntas sugeridas
                </span>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {SUGGESTIONS.map((sug, i) => (
                    <button
                      ...
                      className="text-left text-xs bg-card hover:bg-accent hover:text-accent-foreground p-3 rounded-xl border border-border text-foreground transition-all cursor-pointer group"
                    >
                      <span>{sug}</span>
                    </button>
                  ))}
            ...
            messages.map((msg) => (
              ...
              >
                {/* User Message: Sleek right-aligned bubble with default shadcn tokens */}
                {msg.role === "user" ? (
                  <div className="rounded-2xl rounded-tr-xs bg-muted text-foreground border border-border px-4 py-2.5 max-w-[85%] text-sm leading-relaxed shadow-xs">
                    {msg.content}
                  </div>
                ) : (
                  ...
                  <div className="w-full max-w-3xl space-y-2">
                    ...
                    {msg.hitlTicket && (
                      <div className="mt-3 p-3.5 rounded-xl bg-destructive/10 border border-destructive/20 text-xs flex flex-col gap-2">
                        <div className="flex items-center justify-between text-destructive font-medium">
                        ...
                        <button
                          ...
                          className="self-start text-[11px] font-medium text-primary hover:underline cursor-pointer flex items-center gap-1 mt-1"
                        >
                    ...
                    <div className="flex items-center gap-1 pt-1 text-muted-foreground">
                      <Button
                        ...
                        className={`size-7 rounded-md hover:text-foreground hover:bg-muted ${
                          msg.feedbackGiven ? "text-emerald-500" : ""
                        }`}
